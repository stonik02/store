        ...
        <div class="album py-5 bg-light">
            <div class="container">
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                    <li v-for="{ image, name, description, id , price} in products" :key="name">
                        <div class="col">
                            <div class="card shadow-sm">
                                <router-link :to="`/product/${id}`">
                                    <img :src="ip + image" class="bd-placeholder-img card-img-top" width="100%" height="250"
                                        alt="Пример" aria-label="Placeholder: Thumbnail">
                                </router-link>
                                <text x="50%" y="50%" fill="#eceeef" dy=".3em" style="text-align: center;">{{ name }}</text>

                                <div class="card-body">
                                    <p class="card-text" style="text-align: center;">{{ description }}</p>
                                    <strong class="card-text" style="text-align: center;">{{ price }}</strong>
...
export default {
    ...
    mounted() {
        axios.get(localStorage.ip + `categories/${this.$route.params.id}`).then((response) => {
